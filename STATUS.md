...
| Application identity portable contracts | `PASS` (Core 110, Service 125; 235/235 per timezone run) |
| Isolated Windows VM | `BLOCKED_WINDOWS_VM` |
| WiX license eligibility | `WIX_ELIGIBILITY_CONFIRMED_EULA_ACCEPTANCE_PENDING` |

Task 5 fresh local evidence is 235/235 passing tests in both default and `TZ=UTC` runs: Core 110 and Guard.Service 125. Locked restore, format, Release build, and diff check pass; Release build has zero warnings/errors. Hosted PR #5 run `34701889964` is the current merged clean run.
